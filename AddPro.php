...
<body>
    ...
    if (isset($_POST['btnAdd'])) {
        ...
        if ($result == '') {
            ...
            $signu1 = $re->execute(array("$shop", "$Id", "$Quantity","$Price", "$udi", "$Date")); 
    ...
    <!-- ?> -->
    <form id="form1" method="POST" action="AddPro.php" role="form">
    ...
    if($re->num_rows>0):
       while($row=$re->fetch_assoc()):?>
            ...
            endwhile;
             ...
             <select style = " margin-top : 20px; margin-bottom: 50px; padding: 5px" name="Sid" id="Sid">
              <?php  
                include_once 'connect1.php';
                $c=new connect();
             $dblink = $c->connectToMySQL();
   ...
   //  $re->execute(array("$Id"));
    if($re->num_rows>0):
       while($row=$re->fetch_assoc()):?>
                ...
                <!-- <a class = "dropdown-item" value ="<?=$row['Sid']?>"><?=$row['SName']?></a> -->
